...
- **Tail `no-article` runs (clean break mid-wave) mean transient render degradation, not post deletions** — probe page health first (navigate + article count + login chip via `cloak_evaluate`), then carry the unfired remainder as `wave-*-carry.json` immediately and point `pending_verify` at the carry's own fire log. Fast fails (~5-6s vs normal 13-15s) that start after a clean hit streak are the signature.
...
- **Draft-batch saves (2026-09-24)**: `post_draft.py` takes ~45-50s per call (first call of a session ~95s cold) - a batch of 8 is ~6-7 min, so run batch saves as a background script (foreground would hit the ~420s terminal cap mid-batch). Verify the batch by its texts in the drawer (`ops/x_drafts_inspect.py`), not just the per-call OK. Draft saves are private (no public action) - safe inside the dark window; the daily 03:00Z human-lines job relies on this.
...
- **QUOTE ARM (owner pivot 2026-09-23): the fire arm.** Quote posts = original posts -> they COUNT toward verified Home Timeline impressions (replies don't) AND carry QuoteWeight 5.0. Measured 2026-09-23: 66 measured replies -> 19 total likes / 59 zeros / 0 countable impressions - the reply arm was noise, so the worker now fires the composed batch as QUOTES (`run_quote_wave.sh --detach`, quote_driver.py; per-post outcome + quote_url logged; conversations stay replies via run_wave.sh). No draft step for quotes. Single-quote tool: `quote_publish.py <target_url> "<comment>" [--dry]`. No double-quoting an account in a day.
...
- **Genre lock (owner directive 2026-09-23, hard):** ONLY tech/AI, anime, and games. No sports, no movies/TV, no music/K-pop, no Bollywood, no celebrity, no politics, no general news. Harvest queries stay inside those three. Off-lock posts are a SKIP — no reply, no quote, no draft. A thin in-lock wave beats a full off-genre wave.
- **Country peak bend (owner directive 2026-09-23):** check UTC before harvest. Tilt ~40% of lanes toward the awake country's timeline, still inside the genre lock, and run ALL THREE genres there (tech/AI, anime, games). Never assign a genre to a country. India is not the anime country. China is not the games country. India 13:30-17:30 UTC. SF/Pacific is the America bend that matters: bend hard 06:30-10:00 UTC and 23:00-23:30 UTC. East Coast 23:00-03:00 is mostly sleep-window. China 12:00-16:00 UTC (lang:zh + TW/HK/overseas, all three genres; X is thin for mainland). Overlap 13:30-16:00: split India+China, all three genres inside each share. Off-peak: global. Bend is whose timeline, not a fake accent. Unreadable post = skip.
