#!/usr/bin/env python3
"""Measure our own recent replies via search route."""
import asyncio, json
...
JOBS = [
    ("q1", "https://x.com/search?q=from%3Ayour_handle%20filter%3Areplies&f=live"),
    ("q2", "https://x.com/search?q=from%3Ayour_handle%20since%3A2026-09-20&f=live"),
]
    ...
    async with streamable_http_client(URL) as ctx:
        ...
        async with ClientSession(r, w) as s:
            ...
            for tag, u in JOBS:
                ...
                out[tag] = {"url": await ev(s, page, "location.href"), "rows": await ev(s, page, LIST_JS)}
                await asyncio.sleep(0.5)
    json.dump(out, open("/home/ubuntu/x-op/targets/measure-own-2.json", "w"), ensure_ascii=False, indent=1)
    for tag, d in out.items():
        print("=" * 60)
        print("##", tag, d.get("url"))
        for row in (d.get("rows") or [])[:12]:
            print(f'  likes={row.get("likes")} reps={row.get("reps")} | {row.get("text","")[:100]}')
