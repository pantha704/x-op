...
async def main():
    async with streamable_http_client("http://127.0.0.1:8932/mcp") as ctx:
        ...
        async with ClientSession(r, w) as s:
            ...
            # try @gork profile
            await call(s, "cloak_navigate", {"page_id": page, "url": "https://x.com/gork"})
            await asyncio.sleep(5)
            for _ in range(4):
                await call(s, "cloak_evaluate", {"page_id": page, "expression": "window.scrollBy(0, 1400); 'ok'"})
                await asyncio.sleep(0.8)
            r3 = await call(s, "cloak_evaluate", {"page_id": page, "expression": "document.body.innerText.slice(0, 8000)"})
