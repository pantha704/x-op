#!/usr/bin/env python3
"""Round 7 (e-slice): capture URLs + text for caseohOOC rewind 99 posts."""
import asyncio, json
...
SEARCH_JS = r"""JSON.stringify((() => {
  ...
  return {n: arts.length, posts: arts.slice(0, 14).map(a => {
    ...
    return {who: h ? h.innerText.split('\n')[0] : '', url: link, txt: t ? t.innerText.slice(0, 360) : ''};
  })};
})())"""

JOBS = [
    ("ooc_rewind", "https://x.com/search?q=from%3AcaseohOOC%20rewind%2099&src=typed_query&f=live", SEARCH_JS),
    ("rewind_caseoh", "https://x.com/search?q=%22rewind%2099%22%20caseoh&src=typed_query&f=live", SEARCH_JS),
    ("ooc_recent", "https://x.com/search?q=from%3AcaseohOOC%20since%3A2026-09-20&src=typed_query&f=live", SEARCH_JS),
]
...
async def main():
    ...
    json.dump(out, open("/home/ubuntu/x-op/targets/ctx-e-rewind2-out.json", "w"), ensure_ascii=False, indent=1)
